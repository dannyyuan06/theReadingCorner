    ...
    <>
      ...
      <div className={styles.titles}>
        <Link href={`/books/${book.bookid}`}>
          <h3 className={styles.title}>{cut(book.title, 20)}</h3>
        </Link>
        <h3>AUTHOR: {cut(book.author, 25)}</h3>
        <h3>
          START DATE:{" "}
          <span>
            {userbook.dateStarted.toDateString().split(" ").slice(1).join(" ")}
          </span>
        </h3>
        <h3>
          FINISH DATE:{" "}
          {cut(
            userbook.dateFinished.valueOf() === 0
              ? "Reading"
      ...
      <div className={styles.progress}>
        ...
        <div className={styles.progressText}>
          Reading
          <span> {userbook.page}</span>
          <span style={{ color: "var(--theme-light-grey)" }}>
            /{book.pageCount}{" "}
          </span>
          pages
        </div>
