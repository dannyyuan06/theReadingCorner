...
describe("Field component", () => {
  ...
  it("should render the Field component with the correct validation message for long username", async () => {
    render(
      <Field
        ...
        formData={formDataTest}
        setFormData={jest.fn()}
        validation={usernameFormValidation}
        type={type}
        isCorrect={isCorrectMockFalse}
      />
    );
    ...
    await waitFor(() => {
      expect(
        screen.getByText("Username must be between 8 and 16 valid characters.")
      ).toBeInTheDocument();
    });
  });

  it("should render the Field component with the correct validation message for short username", async () => {
    render(
      <Field
        name={name}
        formData={formDataTest}
  ...
  it("should render the Field component with the correct validation message for less than 8 character password", async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByText("Password must be between 8 and 16 valid characters.")
      ).toBeInTheDocument();
    });
  });
  ...
  it("should call the validation function when the input value changes", async () => {
    ...
    await waitFor(() => {
      expect(validation).toHaveBeenCalledWith("johndoe");
    });
  });
